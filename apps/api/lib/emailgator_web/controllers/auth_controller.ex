defmodule EmailgatorWeb.AuthController do
  ...
  def request(conn, _params) do
    ...
    Logger.info(
      "🔍 OAuth Request - GOOGLE_OAUTH_REDIRECT_URL env: #{inspect(System.get_env("GOOGLE_OAUTH_REDIRECT_URL"))}"
    )
  ...
  def callback(conn, %{"code" => _code} = params) do
    ...
    if get_session(conn, :oauth_redirect_uri) == nil do
      Logger.warning(
        "⚠️  No session found - this might be a reused authorization code. Authorization codes are single-use and expire quickly."
      )

      Logger.warning(
        "⚠️  If you're seeing 'invalid_grant', you need to start a fresh OAuth flow from the beginning."
      )
    end
    ...
    Logger.info(
      "🔍 OAuth Callback - redirect_uri from session: #{inspect(get_session(conn, :oauth_redirect_uri))}"
    )
    ...
    Logger.info(
      "🔍 OAuth Callback - GOOGLE_OAUTH_REDIRECT_URL env: #{inspect(System.get_env("GOOGLE_OAUTH_REDIRECT_URL"))}"
    )
    ...
    Logger.info(
      "🔍 Callback config: client_id = #{inspect(Keyword.get(callback_config, :client_id))}"
    )

    Logger.info(
      "🔍 Callback config: redirect_uri = #{inspect(Keyword.get(callback_config, :redirect_uri))}"
    )
